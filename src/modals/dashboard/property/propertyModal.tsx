...
import {
  ...
  handleView,
  handleEdit,
} from './common/components/propertyTableComponents'
...
const PropertyModal = () => {
  ...
  const { currentPage, visibleData, handlePageChange, totalPages } = useFilteredPagination(
    propertyModalData.propertyData,
    '',
    itemsPerPage,
  )
    ...
    return (
      <div className='flex space-x-2'>
        <PrIconV2
          ...
          color='blue'
          onClick={() => handleView(props?.rowData?.property_id)}
        ></PrIconV2>
  ...
  const fetchData = async () => {
    try {
      ...
      if (typeof data === 'object' && data !== null) {
        const dataMessageWithAddress =
          data?.responseData['message']?.map((d: any) => ({
            ...d,
            address: {
