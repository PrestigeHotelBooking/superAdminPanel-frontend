...
function PolicyModal({ id }: { id: string }){
    ...
    return(
        <div className="mb-64">
            <PrTextArea label={"General Policy"} value={policyModalData.generalPolicy} onChange={(e)=>{handleState({ generalPolicy:e.target.value  })}}></PrTextArea>
            <PrTextArea label={`Cancellation Policy`} onChange={(e)=>{handleState({ cancellationPolicy:e.target.value })}} value={policyModalData.cancellationPolicy}></PrTextArea>
            <PrTextArea label={`Payment Policy`} onChange={(e)=>{ handleState({ paymentPolicy:e.target.value }) }} value={policyModalData.paymentPolicy}></PrTextArea>
