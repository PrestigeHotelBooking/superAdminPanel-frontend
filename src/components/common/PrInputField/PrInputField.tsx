...
const PrInputField: React.FC<PrInputFieldV2Props> = ({ label, className, type, value, ...inputProps }) => {
  ...
  const inputClasses = `mt-2 px-3 py-2 block w-[20.5rem] h-[3.5rem] border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm ${
    className || ''
  }`;
